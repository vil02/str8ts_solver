import typing


class Sample(typing.NamedTuple):
    size: tuple[int, int]
    blocked: set[tuple[int, int]]
    known: dict[tuple[int, int], int]
    solved: dict[tuple[int, int], int] | None
    blank_str: str
    solved_str: str | None


# source:
# blank: https://en.wikipedia.org/wiki/File:Str8ts9x9_Gentle_PUZ.png
# solved: https://en.wikipedia.org/wiki/Str8ts#/media/File:Str8ts9x9_Gentle_SOL.png
# pylint: disable=duplicate-code
SAMPLE_1 = Sample(
    size=(9, 9),
    blocked={
        (0, 0),
        (1, 0),
        (7, 0),
        (8, 0),
        (4, 1),
        (5, 1),
        (3, 2),
        (4, 2),
        (3, 3),
        (7, 3),
        (8, 3),
        (0, 4),
        (8, 4),
        (0, 5),
        (1, 5),
        (5, 5),
        (4, 6),
        (5, 6),
        (3, 7),
        (4, 7),
        (0, 8),
        (1, 8),
        (7, 8),
        (8, 8),
    },
    known={
        (4, 0): 5,
        (7, 0): 3,
        (1, 1): 6,
        (6, 1): 1,
        (4, 2): 8,
        (0, 3): 9,
        (3, 3): 4,
        (8, 3): 5,
        (5, 4): 3,
        (5, 5): 9,
        (7, 5): 4,
        (0, 6): 4,
        (2, 6): 3,
        (7, 6): 6,
        (2, 7): 1,
        (2, 8): 8,
        (8, 8): 2,
    },
    solved={
        (0, 1): 8,
        (0, 2): 7,
        (0, 3): 9,
        (0, 6): 4,
        (0, 7): 3,
        (1, 1): 6,
        (1, 2): 5,
        (1, 3): 8,
        (1, 4): 7,
        (1, 6): 1,
        (1, 7): 2,
        (2, 0): 9,
        (2, 1): 5,
        (2, 2): 6,
        (2, 3): 7,
        (2, 4): 4,
        (2, 5): 2,
        (2, 6): 3,
        (2, 7): 1,
        (2, 8): 8,
        (3, 0): 6,
        (3, 1): 7,
        (3, 4): 1,
        (3, 5): 3,
        (3, 6): 2,
        (3, 8): 5,
        (4, 0): 5,
        (4, 3): 3,
        (4, 4): 2,
        (4, 5): 1,
        (4, 8): 6,
        (5, 0): 8,
        (5, 2): 2,
        (5, 3): 1,
        (5, 4): 3,
        (5, 7): 6,
        (5, 8): 7,
        (6, 0): 7,
        (6, 1): 1,
        (6, 2): 3,
        (6, 3): 2,
        (6, 4): 6,
        (6, 5): 5,
        (6, 6): 8,
        (6, 7): 9,
        (6, 8): 4,
        (7, 1): 2,
        (7, 2): 1,
        (7, 4): 5,
        (7, 5): 4,
        (7, 6): 6,
        (7, 7): 7,
        (8, 1): 3,
        (8, 2): 4,
        (8, 5): 6,
        (8, 6): 7,
        (8, 7): 8,
    },
    blank_str="""+---+---+---+---+---+---+---+---+---+
|###|###|   |   |  !|   |   |###|###|
|###|###|   |   | 5 |   |   |#3#|###|
|###|###|   |   |   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+
|   |  !|   |   |###|###|  !|   |   |
|   | 6 |   |   |###|###| 1 |   |   |
|   |   |   |   |###|###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |   |###|###|   |   |   |   |
|   |   |   |###|#8#|   |   |   |   |
|   |   |   |###|###|   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|  !|   |   |###|   |   |   |###|###|
| 9 |   |   |#4#|   |   |   |###|#5#|
|   |   |   |###|   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+
|###|   |   |   |   |  !|   |   |###|
|###|   |   |   |   | 3 |   |   |###|
|###|   |   |   |   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|###|###|   |   |   |###|   |  !|   |
|###|###|   |   |   |#9#|   | 4 |   |
|###|###|   |   |   |###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|  !|   |  !|   |###|###|   |  !|   |
| 4 |   | 3 |   |###|###|   | 6 |   |
|   |   |   |   |###|###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |  !|###|###|   |   |   |   |
|   |   | 1 |###|###|   |   |   |   |
|   |   |   |###|###|   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|###|###|  !|   |   |   |   |###|###|
|###|###| 8 |   |   |   |   |###|#2#|
|###|###|   |   |   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+""",
    solved_str="""+---+---+---+---+---+---+---+---+---+
|###|###|   |   |  !|   |   |###|###|
|###|###| 9 | 6 | 5 | 8 | 7 |#3#|###|
|###|###|   |   |   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+
|   |  !|   |   |###|###|  !|   |   |
| 8 | 6 | 5 | 7 |###|###| 1 | 2 | 3 |
|   |   |   |   |###|###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |   |###|###|   |   |   |   |
| 7 | 5 | 6 |###|#8#| 2 | 3 | 1 | 4 |
|   |   |   |###|###|   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|  !|   |   |###|   |   |   |###|###|
| 9 | 8 | 7 |#4#| 3 | 1 | 2 |###|#5#|
|   |   |   |###|   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+
|###|   |   |   |   |  !|   |   |###|
|###| 7 | 4 | 1 | 2 | 3 | 6 | 5 |###|
|###|   |   |   |   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|###|###|   |   |   |###|   |  !|   |
|###|###| 2 | 3 | 1 |#9#| 5 | 4 | 6 |
|###|###|   |   |   |###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|  !|   |  !|   |###|###|   |  !|   |
| 4 | 1 | 3 | 2 |###|###| 8 | 6 | 7 |
|   |   |   |   |###|###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |  !|###|###|   |   |   |   |
| 3 | 2 | 1 |###|###| 6 | 9 | 7 | 8 |
|   |   |   |###|###|   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|###|###|  !|   |   |   |   |###|###|
|###|###| 8 | 5 | 6 | 7 | 4 |###|#2#|
|###|###|   |   |   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+""",
)

# source:
# https://de.wikipedia.org/wiki/Str8ts#/media/Datei:Str8ts9x9_Medium_SOL_animated.gif
SAMPLE_2 = Sample(
    size=(9, 9),
    blocked={
        (0, 0),
        (4, 0),
        (5, 0),
        (0, 1),
        (8, 1),
        (0, 2),
        (3, 2),
        (8, 2),
        (1, 3),
        (2, 3),
        (1, 4),
        (7, 4),
        (6, 5),
        (7, 5),
        (0, 6),
        (5, 6),
        (8, 6),
        (0, 7),
        (8, 7),
        (3, 8),
        (4, 8),
        (8, 8),
    },
    known={
        (0, 0): 5,
        (1, 0): 1,
        (8, 0): 6,
        (7, 1): 4,
        (3, 2): 9,
        (8, 2): 7,
        (4, 3): 7,
        (6, 3): 5,
        (0, 4): 3,
        (1, 4): 4,
        (2, 4): 6,
        (8, 4): 1,
        (2, 5): 8,
        (6, 5): 1,
        (5, 6): 8,
        (4, 7): 6,
        (6, 7): 4,
        (0, 8): 8,
        (5, 8): 1,
    },
    solved={
        (0, 3): 2,
        (0, 4): 3,
        (0, 5): 4,
        (0, 8): 8,
        (1, 0): 1,
        (1, 1): 3,
        (1, 2): 2,
        (1, 5): 5,
        (1, 6): 6,
        (1, 7): 8,
        (1, 8): 7,
        (2, 0): 3,
        (2, 1): 2,
        (2, 2): 1,
        (2, 4): 6,
        (2, 5): 8,
        (2, 6): 5,
        (2, 7): 7,
        (2, 8): 9,
        (3, 0): 2,
        (3, 1): 1,
        (3, 3): 8,
        (3, 4): 7,
        (3, 5): 6,
        (3, 6): 4,
        (3, 7): 5,
        (4, 1): 5,
        (4, 2): 4,
        (4, 3): 7,
        (4, 4): 8,
        (4, 5): 9,
        (4, 6): 3,
        (4, 7): 6,
        (5, 1): 6,
        (5, 2): 3,
        (5, 3): 4,
        (5, 4): 5,
        (5, 5): 7,
        (5, 7): 2,
        (5, 8): 1,
        (6, 0): 8,
        (6, 1): 7,
        (6, 2): 6,
        (6, 3): 5,
        (6, 4): 9,
        (6, 6): 2,
        (6, 7): 4,
        (6, 8): 3,
        (7, 0): 7,
        (7, 1): 4,
        (7, 2): 5,
        (7, 3): 6,
        (7, 6): 1,
        (7, 7): 3,
        (7, 8): 2,
        (8, 0): 6,
        (8, 3): 3,
        (8, 4): 1,
        (8, 5): 2,
    },
    blank_str="""+---+---+---+---+---+---+---+---+---+
|###|  !|   |   |###|###|   |   |  !|
|#5#| 1 |   |   |###|###|   |   | 6 |
|###|   |   |   |###|###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|###|   |   |   |   |   |   |  !|###|
|###|   |   |   |   |   |   | 4 |###|
|###|   |   |   |   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|###|   |   |###|   |   |   |   |###|
|###|   |   |#9#|   |   |   |   |#7#|
|###|   |   |###|   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|   |###|###|   |  !|   |  !|   |   |
|   |###|###|   | 7 |   | 5 |   |   |
|   |###|###|   |   |   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|  !|###|  !|   |   |   |   |###|  !|
| 3 |#4#| 6 |   |   |   |   |###| 1 |
|   |###|   |   |   |   |   |###|   |
+---+---+---+---+---+---+---+---+---+
|   |   |  !|   |   |   |###|###|   |
|   |   | 8 |   |   |   |#1#|###|   |
|   |   |   |   |   |   |###|###|   |
+---+---+---+---+---+---+---+---+---+
|###|   |   |   |   |###|   |   |###|
|###|   |   |   |   |#8#|   |   |###|
|###|   |   |   |   |###|   |   |###|
+---+---+---+---+---+---+---+---+---+
|###|   |   |   |  !|   |  !|   |###|
|###|   |   |   | 6 |   | 4 |   |###|
|###|   |   |   |   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|  !|   |   |###|###|  !|   |   |###|
| 8 |   |   |###|###| 1 |   |   |###|
|   |   |   |###|###|   |   |   |###|
+---+---+---+---+---+---+---+---+---+""",
    solved_str="""+---+---+---+---+---+---+---+---+---+
|###|  !|   |   |###|###|   |   |  !|
|#5#| 1 | 3 | 2 |###|###| 8 | 7 | 6 |
|###|   |   |   |###|###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|###|   |   |   |   |   |   |  !|###|
|###| 3 | 2 | 1 | 5 | 6 | 7 | 4 |###|
|###|   |   |   |   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|###|   |   |###|   |   |   |   |###|
|###| 2 | 1 |#9#| 4 | 3 | 6 | 5 |#7#|
|###|   |   |###|   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|   |###|###|   |  !|   |  !|   |   |
| 2 |###|###| 8 | 7 | 4 | 5 | 6 | 3 |
|   |###|###|   |   |   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|  !|###|  !|   |   |   |   |###|  !|
| 3 |#4#| 6 | 7 | 8 | 5 | 9 |###| 1 |
|   |###|   |   |   |   |   |###|   |
+---+---+---+---+---+---+---+---+---+
|   |   |  !|   |   |   |###|###|   |
| 4 | 5 | 8 | 6 | 9 | 7 |#1#|###| 2 |
|   |   |   |   |   |   |###|###|   |
+---+---+---+---+---+---+---+---+---+
|###|   |   |   |   |###|   |   |###|
|###| 6 | 5 | 4 | 3 |#8#| 2 | 1 |###|
|###|   |   |   |   |###|   |   |###|
+---+---+---+---+---+---+---+---+---+
|###|   |   |   |  !|   |  !|   |###|
|###| 8 | 7 | 5 | 6 | 2 | 4 | 3 |###|
|###|   |   |   |   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|  !|   |   |###|###|  !|   |   |###|
| 8 | 7 | 9 |###|###| 1 | 3 | 2 |###|
|   |   |   |###|###|   |   |   |###|
+---+---+---+---+---+---+---+---+---+""",
)

# source:
# puzzle No 8 in: https://www.str8ts.com/STR8TS_9x9_Sample_Pack.pdf
SAMPLE_3 = Sample(
    size=(9, 9),
    blocked={
        (0, 0),
        (1, 0),
        (5, 0),
        (5, 1),
        (3, 2),
        (7, 2),
        (8, 2),
        (0, 3),
        (2, 4),
        (6, 4),
        (8, 5),
        (0, 6),
        (1, 6),
        (5, 6),
        (3, 7),
        (3, 8),
        (7, 8),
        (8, 8),
    },
    known={
        (1, 0): 4,
        (2, 1): 9,
        (7, 1): 2,
        (1, 2): 6,
        (3, 3): 5,
        (4, 3): 3,
        (8, 4): 8,
        (1, 5): 7,
        (8, 5): 9,
        (0, 6): 7,
        (3, 7): 1,
        (5, 7): 7,
        (7, 7): 8,
        (8, 7): 6,
    },
    solved={
        (0, 1): 6,
        (0, 2): 5,
        (0, 4): 4,
        (0, 5): 3,
        (0, 7): 2,
        (0, 8): 1,
        (1, 1): 8,
        (1, 2): 6,
        (1, 3): 9,
        (1, 4): 5,
        (1, 5): 7,
        (1, 7): 3,
        (1, 8): 2,
        (2, 0): 6,
        (2, 1): 9,
        (2, 2): 7,
        (2, 3): 8,
        (2, 5): 2,
        (2, 6): 1,
        (2, 7): 4,
        (2, 8): 3,
        (3, 0): 8,
        (3, 1): 7,
        (3, 3): 5,
        (3, 4): 2,
        (3, 5): 4,
        (3, 6): 3,
        (4, 0): 7,
        (4, 1): 5,
        (4, 2): 4,
        (4, 3): 3,
        (4, 4): 1,
        (4, 5): 6,
        (4, 6): 2,
        (4, 7): 9,
        (4, 8): 8,
        (5, 2): 2,
        (5, 3): 4,
        (5, 4): 3,
        (5, 5): 1,
        (5, 7): 7,
        (5, 8): 6,
        (6, 0): 1,
        (6, 1): 4,
        (6, 2): 3,
        (6, 3): 2,
        (6, 5): 8,
        (6, 6): 6,
        (6, 7): 5,
        (6, 8): 7,
        (7, 0): 3,
        (7, 1): 2,
        (7, 3): 6,
        (7, 4): 7,
        (7, 5): 5,
        (7, 6): 4,
        (7, 7): 8,
        (8, 0): 2,
        (8, 1): 3,
        (8, 3): 7,
        (8, 4): 8,
        (8, 6): 5,
        (8, 7): 6,
    },
    blank_str="""+---+---+---+---+---+---+---+---+---+
|###|###|   |   |   |###|   |   |   |
|###|#4#|   |   |   |###|   |   |   |
|###|###|   |   |   |###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |  !|   |   |###|   |  !|   |
|   |   | 9 |   |   |###|   | 2 |   |
|   |   |   |   |   |###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |  !|   |###|   |   |   |###|###|
|   | 6 |   |###|   |   |   |###|###|
|   |   |   |###|   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+
|###|   |   |  !|  !|   |   |   |   |
|###|   |   | 5 | 3 |   |   |   |   |
|###|   |   |   |   |   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |###|   |   |   |###|   |  !|
|   |   |###|   |   |   |###|   | 8 |
|   |   |###|   |   |   |###|   |   |
+---+---+---+---+---+---+---+---+---+
|   |  !|   |   |   |   |   |   |###|
|   | 7 |   |   |   |   |   |   |#9#|
|   |   |   |   |   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|###|###|   |   |   |###|   |   |   |
|#7#|###|   |   |   |###|   |   |   |
|###|###|   |   |   |###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |   |###|   |  !|   |  !|  !|
|   |   |   |#1#|   | 7 |   | 8 | 6 |
|   |   |   |###|   |   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |   |###|   |   |   |###|###|
|   |   |   |###|   |   |   |###|###|
|   |   |   |###|   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+""",
    solved_str="""+---+---+---+---+---+---+---+---+---+
|###|###|   |   |   |###|   |   |   |
|###|#4#| 6 | 8 | 7 |###| 1 | 3 | 2 |
|###|###|   |   |   |###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |  !|   |   |###|   |  !|   |
| 6 | 8 | 9 | 7 | 5 |###| 4 | 2 | 3 |
|   |   |   |   |   |###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |  !|   |###|   |   |   |###|###|
| 5 | 6 | 7 |###| 4 | 2 | 3 |###|###|
|   |   |   |###|   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+
|###|   |   |  !|  !|   |   |   |   |
|###| 9 | 8 | 5 | 3 | 4 | 2 | 6 | 7 |
|###|   |   |   |   |   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |###|   |   |   |###|   |  !|
| 4 | 5 |###| 2 | 1 | 3 |###| 7 | 8 |
|   |   |###|   |   |   |###|   |   |
+---+---+---+---+---+---+---+---+---+
|   |  !|   |   |   |   |   |   |###|
| 3 | 7 | 2 | 4 | 6 | 1 | 8 | 5 |#9#|
|   |   |   |   |   |   |   |   |###|
+---+---+---+---+---+---+---+---+---+
|###|###|   |   |   |###|   |   |   |
|#7#|###| 1 | 3 | 2 |###| 6 | 4 | 5 |
|###|###|   |   |   |###|   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |   |###|   |  !|   |  !|  !|
| 2 | 3 | 4 |#1#| 9 | 7 | 5 | 8 | 6 |
|   |   |   |###|   |   |   |   |   |
+---+---+---+---+---+---+---+---+---+
|   |   |   |###|   |   |   |###|###|
| 1 | 2 | 3 |###| 8 | 6 | 7 |###|###|
|   |   |   |###|   |   |   |###|###|
+---+---+---+---+---+---+---+---+---+""",
)


NO_SOLUTION = Sample(
    size=(3, 2),
    blocked={(0, 0), (1, 1), (2, 0)},
    known={(0, 0): 1, (1, 1): 2, (2, 0): 3},
    solved=None,
    blank_str="""+---+---+---+
|###|   |###|
|#1#|   |#3#|
|###|   |###|
+---+---+---+
|   |###|   |
|   |#2#|   |
|   |###|   |
+---+---+---+""",
    solved_str=None,
)

SAMPLE_3_BY_2 = Sample(
    size=(3, 2),
    blocked={(0, 0)},
    known={(0, 0): 1, (1, 1): 1},
    solved={(0, 1): 3, (1, 0): 2, (1, 1): 1, (2, 0): 3, (2, 1): 2},
    blank_str="""+---+---+---+
|###|   |   |
|#1#|   |   |
|###|   |   |
+---+---+---+
|   |  !|   |
|   | 1 |   |
|   |   |   |
+---+---+---+""",
    solved_str="""+---+---+---+
|###|   |   |
|#1#| 2 | 3 |
|###|   |   |
+---+---+---+
|   |  !|   |
| 3 | 1 | 2 |
|   |   |   |
+---+---+---+""",
)

FULLY_BLOCKED_COLUMN = Sample(
    size=(2, 3),
    blocked={(0, 0), (0, 1), (0, 2)},
    known={(1, 0): 1, (1, 2): 3},
    solved={(1, 0): 1, (1, 1): 2, (1, 2): 3},
    blank_str="""+---+---+
|###|  !|
|###| 1 |
|###|   |
+---+---+
|###|   |
|###|   |
|###|   |
+---+---+
|###|  !|
|###| 3 |
|###|   |
+---+---+""",
    solved_str="""+---+---+
|###|  !|
|###| 1 |
|###|   |
+---+---+
|###|   |
|###| 2 |
|###|   |
+---+---+
|###|  !|
|###| 3 |
|###|   |
+---+---+""",
)
